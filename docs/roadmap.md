...
- Split `/fat/bin/desktop` into a tiny userspace window server and freestanding
  GUI client processes, then start moving utilities to the GUI2/displayd path
  with `/fat/bin/notes` and `/fat/bin/calc`. The desktop launches those ELFs in
  the background, receives fixed-size GUI IPC messages describing windows,
  labels, buttons, and text updates, renders the widgets, and sends click/close
  events back to the owning process. GUI queues are protected against
...
- Extend `scanf`/`sscanf`/`fscanf` with scansets, inverted scansets, simple
...
- Add first-pass `/fat/bin/expr` for script arithmetic, comparisons, string
...
- Add shell job references and inspection polish: `%+`, `%-`, numeric `%N`,
  `jobs -l`, and built-in `kill %job` for every pid in a tracked job.
