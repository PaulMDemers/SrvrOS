...
- GUI apps such as `calcgui`, `notesgui`, `textedit`, and `imgedit` are separate
  ring-3 processes.
...
duplicate instance placement and placeholder launch failures. Bottom-right
resize grips now send configure events, and GUI2 clients recreate their
kernel-managed backing surface and redraw at the requested size. Pixel storage
is currently kernel-managed through
`gui_surface_create`,
