...
- Adds config-backed shell services under `/fat/etc/services/*.svc`; the
  ...
  `restart=always`, exposes `service list`, `service log`, `service tail`, and
  `service supervise`, and keeps daemon stdout readable in
  `/fat/var/log/webd.log` with supervisor events in `/fat/var/log/svscan.log`.
