...
def main():
    parser = argparse.ArgumentParser(description="Run Notes storage self-test in srvros.")
    ...
    output = b""
    with tempfile.TemporaryDirectory(prefix="srvros-notes-") as temp_dir:
        disk = os.path.join(temp_dir, "srvros-notes.exfat")
        ...
        try:
            ...
            sock.sendall(b"run /fat/bin/notes --selftest\n")
            output += read_until(sock, b"notes-selftest: ok", args.run_wait)
    ...
    if has_fatal_exception(text):
        print("notes-selftest-smoke: fatal exception detected", file=sys.stderr)
        return 2
    if missing:
        print("notes-selftest-smoke: missing markers:", file=sys.stderr)
        for marker in missing:
            print(f"  {marker}", file=sys.stderr)
        return 3
    print("notes-selftest-smoke: ok")
    return 0
