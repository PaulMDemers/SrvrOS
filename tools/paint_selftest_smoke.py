...
def main():
    parser = argparse.ArgumentParser(description="Run Paint BMP self-test in srvros.")
    ...
    output = b""
    with tempfile.TemporaryDirectory(prefix="srvros-paint-") as temp_dir:
        disk = os.path.join(temp_dir, "srvros-paint.exfat")
        ...
        try:
            ...
            sock.sendall(b"run /fat/bin/paint --selftest\n")
            output += read_until(sock, b"paint-selftest: ok", args.run_wait)
    ...
    if has_fatal_exception(text):
        print("paint-selftest-smoke: fatal exception detected", file=sys.stderr)
        return 2
    if missing:
        print("paint-selftest-smoke: missing markers:", file=sys.stderr)
        for marker in missing:
            print(f"  {marker}", file=sys.stderr)
        return 3
    print("paint-selftest-smoke: ok")
    return 0
