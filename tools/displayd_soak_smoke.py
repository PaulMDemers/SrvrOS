...
def main():
    parser = argparse.ArgumentParser(description="Run displayd launch/move/resize/minimize/close soak smoke.")
    ...
    parser.add_argument("--displayd-wait", type=float, default=75)
    ...
    output = b""
    with tempfile.TemporaryDirectory(prefix="srvros-displayd-soak-") as temp_dir:
        ...
        try:
            ...
            sock.sendall(b"displayd --soak-smoke\n")
    ...
    expected = [
        ...
        "displayd: soak launch phase",
        "displayd: mapped surface window NOTES",
        "displayd: mapped surface window CALC",
        "displayd: mapped surface window GUI2 DEMO",
        "displayd: drag ",
        "displayd: resize ",
        "displayd: remap surface window ",
        "displayd: minimize ",
        "displayd: taskbar restore ",
        "displayd: soak close phase",
        "displayd: soak smoke ok",
        "displayd: exited",
    ]
    missing = [marker for marker in expected if marker not in text]
    if has_fatal_exception(text):
        print("displayd-soak-smoke: fatal exception detected", file=sys.stderr)
        return 2
    if missing:
        print("displayd-soak-smoke: missing markers:", file=sys.stderr)
        for marker in missing:
            print(f"  {marker}", file=sys.stderr)
        return 3

    print("displayd-soak-smoke: ok")
    return 0
