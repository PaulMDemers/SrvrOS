...
def tcp_client(port, index, timeout):
    payload = f"tcpstress-{index}".encode("ascii")
    expected = f"tcpstress-reply-{index}".encode("ascii")
    deadline = time.time() + timeout
    last_error = None
    while time.time() < deadline:
        try:
            with socket.create_connection(("127.0.0.1", port), timeout=2) as sock:
                ...
                if response != expected:
                    raise RuntimeError(f"bad response {response!r}, expected {expected!r}")
                return response
        except (OSError, RuntimeError) as exc:
            last_error = exc
            time.sleep(0.4)
    raise RuntimeError(f"tcpstress client {index} failed: {last_error}")
...
def main():
    parser = argparse.ArgumentParser(description="Verify TCP descriptor lifetime and close ordering.")
    ...
    parser.add_argument("--command-wait", type=float, default=30)
    ...
    responses = []
    client_error = None
    with tempfile.TemporaryDirectory(prefix="srvros-tcpstress-") as temp_dir:
        disk = os.path.join(temp_dir, "srvros-tcpstress.exfat")
        ...
        command = [
            ...
            "-netdev", f"user,id=net0,hostfwd=tcp::{host_port}-10.0.2.15:{guest_port}",
        ...
        try:
            ...
            sock.sendall(f"tcpstress {guest_port} {args.clients}\n".encode("ascii"))
            output += read_until(sock, b"tcpstress: listening", args.command_wait)
            for index in range(1, args.clients + 1):
                try:
                    responses.append(tcp_client(host_port, index, args.tcp_wait))
                except Exception as exc:
                    client_error = exc
                    output += read_for(sock, 2)
                    break
                output += read_until(sock, f"tcpstress: client {index} ok".encode("ascii"), args.command_wait)
            output += read_until(sock, b"tcpstress: ok", args.command_wait)
    ...
    missing = []
    if "tcpstress: ok" not in text:
        missing.append("tcpstress: ok")
    if "tcpstress: fail" in text:
        missing.append("tcpstress failure")
    if client_error is not None:
        missing.append(f"host client error: {client_error}")
    for index in range(1, args.clients + 1):
        marker = f"tcpstress: client {index} ok"
        if marker not in text:
            missing.append(marker)
        expected = f"tcpstress-reply-{index}".encode("ascii")
        if expected not in responses:
            missing.append(expected.decode("ascii"))
    if has_fatal_exception(text):
        print("tcpstress-smoke: fatal exception detected", file=sys.stderr)
        return 2
    if missing:
        print("tcpstress-smoke: missing markers:", file=sys.stderr)
        for marker in missing:
            print(f"  {marker}", file=sys.stderr)
        return 3

    print("tcpstress-smoke: ok")
    return 0
