...
def main():
    ...
    default_names = ["hello", "cat", "webd", "httpget", "udpdns", "udpecho", "netstat", "ifconfig", "route",
                     "arp", "ping", "host", "netcheck", "netabi", "tcpstress", "sysabi", "spin", "ui", "desktop", "displayd", "surfacedemo", "gui2demo", "calcgui", "calc2", "notesgui", "notes2", "textedit",
                     "imgedit"]
    app_names = []
    app_data = {}
    app_sources = {}
    ...
    node_http_demo = (
        ...
        b"const root = '/fat/www';\n"
        b"process.on('uncaughtException', () => {});\n"
        ...
        b"};\n"
        b"function clean(url) {\n"
        b"  url = String(url || '/');\n"
        b"  const q = url.indexOf('?');\n"
        b"  if (q >= 0) url = url.slice(0, q);\n"
        b"  if (url === '/' || url === '') return '/index.html';\n"
        b"  if (url.indexOf('..') >= 0 || url.indexOf('\\\\') >= 0) return null;\n"
        b"  if (url[0] !== '/') url = '/' + url;\n"
        b"  return url;\n"
        b"}\n"
        b"function extname(path) {\n"
        b"  const slash = path.lastIndexOf('/');\n"
        b"  const dot = path.lastIndexOf('.');\n"
        b"  return dot > slash ? path.slice(dot) : '';\n"
        b"}\n"
        b"const server = http.createServer((req, res) => {\n"
        b"  req.on('error', () => {});\n"
        b"  res.on('error', () => {});\n"
        b"  const rel = clean(req.url);\n"
        b"  if (rel === null) {\n"
        b"    res.statusCode = 400;\n"
        b"    res.setHeader('Content-Type', 'text/plain; charset=utf-8');\n"
        b"    res.end('bad request\\n');\n"
        b"    return;\n"
        b"  }\n"
        b"  try {\n"
        b"    const body = fs.readFileSync(root + rel);\n"
        b"    res.statusCode = 200;\n"
        b"    res.setHeader('Content-Type', mime[extname(rel)] || 'application/octet-stream');\n"
        b"    res.setHeader('Content-Length', String(body.length));\n"
        b"    res.end(body);\n"
        b"  } catch (err) {\n"
        ...
        b"  }\n"
        b"});\n"
        b"server.on('clientError', (err, socket) => {\n"
        b"  try { socket.end('HTTP/1.1 400 Bad Request\\r\\nConnection: close\\r\\n\\r\\n'); } catch (e) {}\n"
        b"});\n"
        b"server.listen(8080, '0.0.0.0', () => console.log('NODEHTTP-LISTEN 8080'));\n"
    )
    files.append(("bin-node-http-demo.js", allocate_clusters(len(node_http_demo)), node_http_demo))
