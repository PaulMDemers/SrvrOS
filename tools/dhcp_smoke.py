...
def main():
    parser = argparse.ArgumentParser(description="Verify DHCP plus webd on srvros.")
    ...
    parser.add_argument("--dhcp-wait", type=float, default=8)
    ...
    response = b""
    http_error = None
    with tempfile.TemporaryDirectory(prefix="srvros-dhcp-") as temp_dir:
        disk = os.path.join(temp_dir, "srvros-dhcp.exfat")
        ...
        try:
            ...
            try:
                ...
                http_error = exc
            sock.sendall(b"write /fat/www/index.html \"updated page\"\n")
            output += read_until(sock, b" $ ", 5)
            try:
    ...
    if b"HTTP/1.1 200 OK" not in response:
        missing.append("HTTP/1.1 200 OK")
    if b"<h1>srvros</h1>" not in response:
        missing.append("<h1>srvros</h1>")
    if b"updated page" not in response:
        missing.append("updated page")
    if http_error is not None:
        missing.append(str(http_error))
    if has_fatal_exception(text):
        print("dhcp-smoke: fatal exception detected", file=sys.stderr)
        return 2
    if missing:
        print("dhcp-smoke: missing markers:", file=sys.stderr)
