...
def main():
    parser = argparse.ArgumentParser(description="Run File Manager filesystem action self-test in srvros.")
    ...
    output = b""
    with tempfile.TemporaryDirectory(prefix="srvros-fileman-") as temp_dir:
        disk = os.path.join(temp_dir, "srvros-fileman.exfat")
        ...
        try:
            ...
            sock.sendall(b"run /fat/bin/fileman --selftest\n")
            output += read_until(sock, b"fileman-selftest: ok", args.run_wait)
    ...
    if has_fatal_exception(text):
        print("fileman-selftest-smoke: fatal exception detected", file=sys.stderr)
        return 2
    if missing:
        print("fileman-selftest-smoke: missing markers:", file=sys.stderr)
        for marker in missing:
            print(f"  {marker}", file=sys.stderr)
        return 3
    print("fileman-selftest-smoke: ok")
    return 0
