...
def main():
    parser = argparse.ArgumentParser(
        description="Run a srvros Text Edit shared file-dialog QMP smoke test.")
    ...
    serial_port = choose_port(46001, 50000)
    qmp_port = choose_port(50001, 54000)
    ...
    output = b""
    with tempfile.TemporaryDirectory(prefix="srvros-displayd-dialog-") as temp_dir:
        disk = os.path.join(temp_dir, "srvros-displayd-dialog.exfat")
        ...
        try:
            ...
            output += run_shell(serial,
                "write /fat/home/dialog-open.txt opened-from-open-dialog",
                args.shell_wait)
            serial.sendall(b"gui --textedit-dialog-smoke\n")
            output += read_until(serial, b"displayd: root backbuffer ready",
                args.ready_wait)
            output += read_until(serial,
                b"displayd: launched textedit-dialog pid=", args.map_wait)
            output += read_until(serial,
                b"displayd: mapped surface window TEXT EDIT", args.map_wait)
            output += read_until(serial, b"textedit: configure", args.map_wait)

            mouse = Mouse(qmp)
            # Text Edit frame starts at (220,110); client origin is (221,135).
            mouse.click(250, 195)
            send_qmp_text(qmp, "saved from dialog", args.key_delay)
            output += read_until(serial, b"displayd: focus TEXT EDIT",
                args.action_wait)
            ...
            time.sleep(0.5)
            mouse.click(500, 484)
            output += read_until(serial, b"textedit: save-as", args.action_wait)

            # Open toolbar button, type the known filename in the modal textbox,
            # and accept through the modal OPEN button.
            mouse.click(283, 484)
            time.sleep(0.5)
            mouse.click(363, 446)
            send_qmp_text(qmp, "dialog-open.txt", args.key_delay)
            mouse.click(500, 484)
            output += read_until(serial, b"textedit: open /fat/home/dialog-open.txt",
                args.action_wait)

            # Close Text Edit, then exit displayd through the on-screen Exit button.
            mouse.click(233, 116)
            ...
            output += read_until(serial, b"displayd: shutdown complete",
                args.final_wait)
            output += read_until(serial, b" $ ", args.shell_wait)
            serial.sendall(b"cat /fat/home/dialog-saveas-target.txt\n")
            output += read_until(serial, b"saved from dialog", args.action_wait)
        finally:
            try:
                process.terminate()
    ...
    expected = [
        ...
        "textedit: save-as",
        "saved from dialog",
        "textedit: open /fat/home/dialog-open.txt",
        "textedit: exited",
    ...
    if has_fatal_exception(text):
        print("displayd-textedit-dialog-smoke: fatal exception detected",
            file=sys.stderr)
        return 2
    if missing:
        print("displayd-textedit-dialog-smoke: missing markers:",
            file=sys.stderr)
        for marker in missing:
            print(f"  {marker}", file=sys.stderr)
        return 3

    print("displayd-textedit-dialog-smoke: ok")
    return 0
