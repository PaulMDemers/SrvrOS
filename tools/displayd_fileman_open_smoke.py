...
def main():
    parser = argparse.ArgumentParser(
        description="Run a srvros File Manager open-with QMP smoke test.")
    ...
    parser.add_argument("--action-wait", type=float, default=10)
    ...
    serial_port = choose_port(62001, 65000)
    qmp_port = choose_port(65001, 65530)
    ...
    output = b""
    with tempfile.TemporaryDirectory(prefix="srvros-displayd-fileman-open-") as temp_dir:
        disk = os.path.join(temp_dir, "srvros-displayd-fileman-open.exfat")
        ...
        try:
            ...
            output += run_shell(serial, "mkdir -p /fat/home/open-smoke", args.shell_wait)
            output += run_shell(serial, "write /fat/home/open-smoke/open-route.bin binary-ish", args.shell_wait)
            output += run_shell(serial, "write /fat/home/open-smoke/open-route.bmp placeholder", args.shell_wait)
            output += run_shell(serial, "write /fat/home/open-smoke/open-route.txt hello-fileman", args.shell_wait)
            serial.sendall(b"gui --fileman-open-smoke\n")
            output += read_until(serial, b"displayd: root backbuffer ready",
                args.ready_wait)
            output += read_until(serial,
                b"displayd: launched fileman-open pid=", args.map_wait)
            output += read_until(serial,
                b"displayd: mapped surface window FILES", args.map_wait)
            output += read_until(serial, b"fileman: configure", args.map_wait)

            mouse = Mouse(qmp)
            # File Manager is placed like the dock-launched FILES app. The
            # prepared /fat/home entries sort as bin, bmp, txt.
            row_x = 540
            first_row_y = 157
            row_h = 22
            open_button_x = 647
            ...
            mouse.click(row_x, first_row_y + 2 * row_h)
            mouse.click(open_button_x, open_button_y)
            output += read_until(serial, b"fileman: open TEXT EDIT", args.action_wait)
            output += read_until(serial,
                b"displayd: mapped surface window TEXT EDIT", args.map_wait)
            mouse.click(227, 126)
            output += read_until(serial, b"textedit: exited", args.action_wait)

            mouse.click(row_x, first_row_y + row_h)
            mouse.click(open_button_x, open_button_y)
            output += read_until(serial, b"fileman: open PAINT", args.action_wait)
            output += read_until(serial,
                b"displayd: mapped surface window PAINT", args.map_wait)
            mouse.click(267, 156)
            output += read_until(serial, b"paint: exited", args.action_wait)

            mouse.click(row_x, first_row_y)
            mouse.click(open_button_x, open_button_y)
            output += read_until(serial, b"fileman: unsupported file", args.action_wait)

            mouse.click(507, 63)
            output += read_until(serial, b"fileman: exited", args.action_wait)
            mouse.click(60, 303)
    ...
    if has_fatal_exception(text):
        print("displayd-fileman-open-smoke: fatal exception detected",
            file=sys.stderr)
        return 2
    if missing:
        print("displayd-fileman-open-smoke: missing markers:",
            file=sys.stderr)
        for marker in missing:
            print(f"  {marker}", file=sys.stderr)
        return 3

    print("displayd-fileman-open-smoke: ok")
    return 0
