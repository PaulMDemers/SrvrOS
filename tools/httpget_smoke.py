...
def main():
    parser = argparse.ArgumentParser(description="Verify outbound TCP connect with /fat/bin/httpget.")
    ...
    parser.add_argument("--boot-wait", type=float, default=35)
    parser.add_argument("--line-wait", type=float, default=15)
    parser.add_argument("--http-wait", type=float, default=35)
    ...
    output = b""
    with tempfile.TemporaryDirectory(prefix="srvros-httpget-") as temp_dir:
        disk = os.path.join(temp_dir, "srvros-httpget.exfat")
        ...
        try:
            ...
            command_line = f"httpget {args.host} {args.path}\n".encode("ascii")
            sock.sendall(command_line)
            output += read_until(sock, b" $ ", args.http_wait)
    ...
    if "HTTP/" not in text:
        missing.append("HTTP response")
    if "httpget: connect" in text or "httpget: read" in text or "httpget: write" in text:
        missing.append("httpget error")
    if has_fatal_exception(text):
        print("httpget-smoke: fatal exception detected", file=sys.stderr)
        return 2
    if missing:
        print("httpget-smoke: missing markers:", file=sys.stderr)
        for marker in missing:
            print(f"  {marker}", file=sys.stderr)
        return 3

    print("httpget-smoke: ok")
    return 0
