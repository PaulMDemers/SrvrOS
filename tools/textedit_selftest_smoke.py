...
def main():
    parser = argparse.ArgumentParser(description="Run Text Edit storage self-test in srvros.")
    ...
    parser.add_argument("--run-wait", type=float, default=30)
    ...
    output = b""
    with tempfile.TemporaryDirectory(prefix="srvros-textedit-") as temp_dir:
        disk = os.path.join(temp_dir, "srvros-textedit.exfat")
        ...
        try:
            ...
            sock.sendall(b"run /fat/bin/textedit --selftest\n")
            output += read_until(sock, b"textedit-selftest: ok", args.run_wait)
            output += read_until(sock, b"srv> ", args.run_wait)
    ...
    if has_fatal_exception(text):
        print("textedit-selftest-smoke: fatal exception detected", file=sys.stderr)
        return 2
    if missing:
        print("textedit-selftest-smoke: missing markers:", file=sys.stderr)
        for marker in missing:
            print(f"  {marker}", file=sys.stderr)
        return 3
    print("textedit-selftest-smoke: ok")
    return 0
