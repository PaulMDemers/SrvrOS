...
def main():
    parser = argparse.ArgumentParser(description="Run the displayd GUI app-exit notice smoke test.")
    ...
    parser.add_argument("--displayd-wait", type=float, default=35)
    ...
    output = b""
    with tempfile.TemporaryDirectory(prefix="srvros-displayd-app-exit-") as temp_dir:
        ...
        try:
            ...
            sock.sendall(b"displayd --app-exit-smoke\n")
            output += read_until(sock, b"displayd: exited", args.displayd_wait)
    ...
    expected = [
        ...
        "displayd: launched guifail pid=",
        "guifail: start",
        "displayd: mapped surface window GUI FAIL",
        "guifail: exiting status=23",
        "displayd: remove GUI FAIL reason=exit pid=",
        " status=23",
        "displayd: notice GUI FAIL exit status 23",
        "displayd: app-exit smoke ok",
        "displayd: exited",
    ]
    missing = [marker for marker in expected if marker not in text]
    if has_fatal_exception(text):
        print("displayd-app-exit-smoke: fatal exception detected", file=sys.stderr)
        return 2
    if missing:
        print("displayd-app-exit-smoke: missing markers:", file=sys.stderr)
        for marker in missing:
            print(f"  {marker}", file=sys.stderr)
        return 3

    print("displayd-app-exit-smoke: ok")
    return 0
