...
def main():
    parser = argparse.ArgumentParser(description="Verify displayd framebuffer-console owner release.")
    ...
    parser.add_argument("--probe-wait", type=float, default=25)
    ...
    output = b""
    with tempfile.TemporaryDirectory(prefix="srvros-displayd-owner-") as temp_dir:
        ...
        try:
            ...
            sock.sendall(b"displayd --owner-crash-smoke\n")
            output += read_until(sock, b" $ ", args.probe_wait)
            sock.sendall(b"/fat/bin/guifail --console-muted\n")
            output += read_until(sock, b"guifail: console-muted=0", args.probe_wait)
    ...
    expected = [
        "displayd: framebuffer console muted",
        "displayd: root backbuffer ready",
        "displayd: owner crash smoke exiting",
        "guifail: console-muted=0",
    ]
    missing = [marker for marker in expected if marker not in text]
    if has_fatal_exception(text):
        print("displayd-console-owner-smoke: fatal exception detected", file=sys.stderr)
        return 2
    if missing:
        print("displayd-console-owner-smoke: missing markers:", file=sys.stderr)
        for marker in missing:
            print(f"  {marker}", file=sys.stderr)
        return 3

    print("displayd-console-owner-smoke: ok")
    return 0
