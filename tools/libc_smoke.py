...
def main():
    parser = argparse.ArgumentParser(description="Verify focused libc/POSIX behavior probes.")
    ...
    output = b""
    with tempfile.TemporaryDirectory(prefix="srvros-libc-") as temp_dir:
        disk = os.path.join(temp_dir, "srvros-libc.exfat")
        ...
        try:
            ...
            for command_line, marker in [
                ("libcprobe\n", b"libcprobe: ok"),
                ("nodeprobe\n", b"nodeprobe: ok"),
            ]:
                send_serial_line(sock, command_line)
                output += read_until(sock, marker, args.command_wait)
    ...
    expected = [
        "libcprobe: string ok",
        "libcprobe: format/scan/convert ok",
        "libcprobe: locale ok",
        "libcprobe: stdio lines ok",
        "libcprobe: temp/env ok",
        "libcprobe: time/wchar ok",
        "libcprobe: ok",
    ...
    if has_fatal_exception(text):
        print("libc-smoke: fatal exception detected", file=sys.stderr)
        return 2
    if "libcprobe: fail" in text or ("nodeprobe:" in text and "failed" in text):
        print("libc-smoke: probe failure detected", file=sys.stderr)
        return 4
    if missing:
        print("libc-smoke: missing markers:", file=sys.stderr)
