...
USER_CALC2 := build/userspace/calc2.elf
USER_NOTESGUI := build/userspace/notesgui.elf
USER_NOTES2 := build/userspace/notes2.elf
...
USER_CALC2_C := $(shell find userspace/calc2 -type f -name '*.c' 2>/dev/null | LC_ALL=C sort)
USER_CALC2_S := $(shell find userspace/calc2 -type f -name '*.S' 2>/dev/null | LC_ALL=C sort)
USER_CALC2_OBJ := $(USER_CALC2_C:%.c=build/%.c.o) $(USER_CALC2_S:%.S=build/%.S.o)
USER_CALC2_DEP := $(USER_CALC2_C:%.c=build/%.c.d) $(USER_CALC2_S:%.S=build/%.S.d)
...
USER_NOTES2_C := $(shell find userspace/notes2 -type f -name '*.c' 2>/dev/null | LC_ALL=C sort)
USER_NOTES2_S := $(shell find userspace/notes2 -type f -name '*.S' 2>/dev/null | LC_ALL=C sort)
USER_NOTES2_OBJ := $(USER_NOTES2_C:%.c=build/%.c.o) $(USER_NOTES2_S:%.S=build/%.S.o)
USER_NOTES2_DEP := $(USER_NOTES2_C:%.c=build/%.c.d) $(USER_NOTES2_S:%.S=build/%.S.d)
...
$(USER_CALC2): $(ZIG) $(USER_CRT0_OBJ) $(USER_CALC2_OBJ) $(USER_LIB_OBJ) userspace/app.ld
	mkdir -p $(dir $@)
	$(LD) $(USER_APP_LDFLAGS) $(USER_CRT0_OBJ) $(USER_CALC2_OBJ) $(USER_LIB_OBJ) -o $@
...
$(USER_NOTES2): $(ZIG) $(USER_CRT0_OBJ) $(USER_NOTES2_OBJ) $(USER_LIB_OBJ) userspace/app.ld
	mkdir -p $(dir $@)
	$(LD) $(USER_APP_LDFLAGS) $(USER_CRT0_OBJ) $(USER_NOTES2_OBJ) $(USER_LIB_OBJ) -o $@
...
$(INITRAMFS): $(USER_INIT) $(USER_SH) $(USER_LS) $(USER_ECHO) $(USER_WRITE) $(USER_WC) $(USER_CLEAR) $(USER_PS) $(USER_KILL) $(USER_WHICH) $(USER_ENV) $(USER_PWD) $(USER_TRUE) $(USER_FALSE) $(USER_SLEEP) $(USER_DATE) $(USER_TOUCH) $(USER_MKTEMP) $(USER_BASENAME) $(USER_DIRNAME) $(USER_GREP) $(USER_HEAD) $(USER_TAIL) $(USER_TEE) $(USER_MORE) $(USER_UNAME) $(USER_HOSTNAME) $(USER_UPTIME) $(USER_FIND) $(USER_DU) $(USER_DF) $(USER_SORT) $(USER_UNIQ) $(USER_CUT) $(USER_XARGS) $(USER_SEQ) $(USER_REALPATH) $(USER_ID) $(USER_WHOAMI) $(USER_READLINK) $(USER_CMP) $(USER_YES) $(USER_INSTALL) $(USER_DIFF) $(USER_TAR) $(USER_GZIP) $(USER_MINIZIP) $(USER_MINIUNZ) $(USER_PATCH) $(USER_MAKE) $(USER_BYACC) $(USER_SED) $(USER_EXPR) $(USER_PRINTF) $(USER_TR) $(USER_DD) $(USER_ED) $(USER_POSIXUTILS) $(USER_STAT) $(USER_CHMOD) $(USER_CP) $(USER_RM) $(USER_MKDIR) $(USER_MV) $(USER_TAP) $(USER_SVSCAN) $(USER_WEBD) $(USER_HTTPGET) $(USER_UDPDNS) $(USER_UDPECHO) $(USER_NETSTAT) $(USER_IFCONFIG) $(USER_ROUTE) $(USER_ARP) $(USER_PING) $(USER_HOST) $(USER_NETCHECK) $(USER_NETABI) $(USER_TCPSTRESS) $(USER_TCPPROBE) $(USER_SYSABI) $(USER_SPIN) $(USER_FPDEMO) $(USER_UI) $(USER_DESKTOP) $(USER_DISPLAYD) $(USER_SURFACEDEMO) $(USER_GUI2DEMO) $(USER_CALCGUI) $(USER_NOTESGUI) $(USER_NOTES2) $(USER_TEXTEDIT) $(USER_IMGEDIT) $(USER_POSIXDEMO) $(USER_THREADSTRESS) $(USER_EXECDEMO) $(USER_FDPROBE) $(USER_LOCKPROBE) $(USER_TTYDEMO) $(USER_JSONDEMO) $(USER_INIDEMO) $(USER_LINEDEMO) $(USER_SQLITEDEMO) $(USER_ZLIBDEMO) $(USER_LUA) $(USER_UVDEMO) $(USER_LIBUVDEMO) $(USER_NODEPROBE) $(USER_LIBCPROBE) $(USER_TLSPROBE) $(USER_CXXPROBE) $(USER_CXXSTLPROBE) $(EXFAT_IMAGE) $(shell find initramfs -type f 2>/dev/null | LC_ALL=C sort)
	...
	cp $(USER_CALC2) $(INITRAMFS_ROOT)/calc2
	cp $(USER_NOTESGUI) $(INITRAMFS_ROOT)/notesgui
	cp $(USER_NOTES2) $(INITRAMFS_ROOT)/notes2
